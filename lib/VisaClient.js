...
export class VisaClient {
  ...
  constructor(config = {}, prodConfig) {
    ...
    if (!_.isEmpty(prodConfig)) {
      if (!_.isObject(prodConfig)) {
  ...
  async #prodRequest(config, operationId) {
    ...
    let response;
    if (operationId) {
      if (!this.#clientProd[operationId])
        throw new Error(`Invalid operationId: ${operationId}`);
   ...
   * @param {object} query
   * @param {string} [query.merchantId] - Merchant ID.
   * @param {string} [query.storeId] - Store ID.
   * @param {string} [query.name] - Merchant name.
   * @return {Promise<MerchantDetailsResponse>} - The response data or error object.
   */
  async merchantSearch({ merchantId, storeId, name }) {
    const req = {
      params: {
        communityCode: this.#communityCodeProd ?? this.#communityCode,
        merchantId,
        merchantName: name,
